...
f <- list(family = "Times New Roman", size = 34)
f2 <- list(family = "Times New Roman", size = 30)

x <- list(titlefont = f, tickfont = f2, title = "Bias correction factor (PP BFC)", ticks = "outside", dtick = 0.5)
bins <- list(start = 0, end = 2, size = 0.1) 

viridis_mod <- viridis(10, direction = -1)
viridis_mod <- c(rep(viridis_mod[1], 7), viridis_mod, viridis_mod[10])

y1 <- list(title = "Basins (n)",titlefont = f, tickfont = f2, ticks = "outside", orientation = "v", zeroline = FALSE, autorange="reversed")
...
save_image(fig1, file = "MS1 Results/Figure5_BCF_hist.png", width = 350, height = 1000, scale = 4)
