...
bins <- list(start = 0, end = 2.2, size = 0.1) 

viridis_mod <- viridis(11, direction = -1)
viridis_mod <- c(rep(viridis_mod[1], 9), viridis_mod, viridis_mod[11])

y1 <- list(title = "Basins (n)", titlefont = f, dtick = 3, tickfont = f2, ticks = "outside", orientation = "v", zeroline = FALSE, autorange="reversed")
