...
KGE<-data.frame(matrix(0,sum(areas$nbands > 0)*4,5))
colnames(KGE)<-c("Model", "KGE", "r", "Beta", "Gamma")
...
for (j in 1:4) {
  for (i in 1:83) {
    if (areas$nbands[i] != 0){
      ...
      KGE[which(KGE$Model == 0)[1],]<-c(models[j], KGE_i$KGE.value, KGE_i$KGE.elements)
      print(i)
