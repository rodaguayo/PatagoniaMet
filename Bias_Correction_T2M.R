...
for (t2m in t2m_variables) {
  ...
  for (month in sprintf("%02d", 1:12)) {
    ...
    mva_parameters$me  <- me(sim=t2m_sim_m,   obs=t2m_obs_m, na.rm=TRUE)
    mva_parameters$rSD <- rSD(sim=t2m_sim_m,  obs=t2m_obs_m, na.rm=TRUE)
    print (paste0("2. ", t2m, " parameters: Ok"))
  
    if (t2m == "Tmax") {
      ## 2.1 me parameter ------------------------------------------------------------------------------
      model_values  <- data.frame(extract(covariates[[1]], t2m_shape), me_param = mva_parameters$me)[,-1]
      ...
      ## 2.2 rSD parameter -----------------------------------------------------------------------------
      model_values  <- data.frame(extract(covariates[[1]], t2m_shape), rSD_param = mva_parameters$rSD)[,-1]
      model_values$climate_class <- as.factor(model_values$climate_class)
      ...
      ## 2.1 me parameter --------------------------------------------------------------------------
      model_values  <- data.frame(extract(covariates[[2]], t2m_shape), me_param = mva_parameters$me)[,-1]
      model_values$climate_class <- as.factor(model_values$climate_class)
      rf_model      <- randomForest(me_param ~., data = model_values, ntree = 2000, importance = TRUE)
      ...
      ## 2.2 rSD parameter -------------------------------------------------------------------------
      model_values  <- data.frame(extract(covariates[[2]], t2m_shape), rSD_param = mva_parameters$rSD)[,-1]
      model_values$climate_class <- as.factor(model_values$climate_class)
      rf_model      <- randomForest(rSD_param ~., data = model_values, ntree = 2000, importance = TRUE)
